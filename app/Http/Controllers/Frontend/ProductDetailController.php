...
{
	public function getProductDetail(Request $request, $slug)//slug dùng để thân thiện người dùng chuẩn SEo
	{
		$arraySlug = explode('-', $slug);//tách mãng dự trên dấu gạch trong slug
		$id        = array_pop($arraySlug);// gét lấy ký tự cuối cùng của ID cuối 

		if ($id) {//nếu có
			//1. Lấy thông tin sp
			$product = Cache::remember('PRODUCT_DETAIL_'. $id, 60 * 24 * 24, function () use ($id) {
				return Product::with('category:id,c_name,c_slug', 'keywords')->findOrFail($id);//with lấy quan hệ mấy cái trong hoặt,findOrFail($id) tìm theo id
			});

			//2. Xử lý view
			ProcessViewService::view('products', 'pro_view', 'product', $id);//pro_view tăng lượt xem thêm 1

			// 3. Lấy 5 cái  đánh giá mới nhất 
			...
			$ratingsDashboard = Cache::remember('RATING_DASHBOARD_'. $id, 60 * 24 * 24, function () use ($id) {
				return Rating::groupBy('r_number')//gr theo r_number 1 ->5
					->where('r_product_id', $id)
